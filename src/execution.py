...
def evaluate_with_test_code(
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for idx, sample in enumerate(samples):
            ...
            print(f"Submit: {idx}/{len(samples)}")
...
def evaluate_with_test_cases(
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for idx, solution in enumerate(tqdm(solutions, desc='submit solutions')):
            ...
            args = (task_id, prompt, completion, list(set(task_test_cases)), timeout)
            future = executor.submit(check_correctness_with_test_cases, *args)
            print(f"Submit solutions: {idx}")
            futures.append(future)
