...
def get_result_of_sorted_solutions_old(ground_truth_results_list, sorted_solutions_by_task, topks=[1,2,10]):
...
def get_result_of_sorted_solutions(ground_truth_results_list, sorted_solutions_by_task, topks=[1,], return_raw=False):
    # sorted_solutions_by_task {task_id: [([solutions], score), ...]}
    def _count_correct(solutions: list, ground_truth_results: dict) -> int:
        return sum([ground_truth_results[s] for s in solutions])
    ...
    for topk in topks:
        ...
        for task_id in ground_truth_results.keys():
            ...
            if task_id in sorted_solutions_by_task and sorted_solutions_by_task[task_id]:
                ...
                cur_weights = []
                cur_weighted_pass_rates = []
                for solutions, score in sorted_solutions_by_task[task_id]:
                    correct = _count_correct(solutions, ground_truth_results[task_id]) / len(solutions)
                    cur_weighted_pass_rates.append(correct * score)
                    cur_weights.append(score)

                pass_rates.append(1-all_wrong_probability)
            else:
                breakpoint()
                pass_rates.append(random_pass_at_k_by_task[task_id])
                weighted_pass_rates.append(random_pass_at_k_by_task[task_id])
        
        # the avg rate of all tasks
        topk_results[f'pass@{topk}'] = round(statistics.mean(pass_rates), 4)
        if return_raw:
            topk_results_raw[f'pass@{topk}'] = dict(zip(ground_truth_results.keys(), pass_rates))
    if return_raw:
        return topk_results, topk_results_raw
    else:
        return topk_results
